...
Direct favicon discovery shipped on 2026-09-05. It prefers sanitized SVG, uses declared HTML and manifest icons, and normalizes raster images to proportionally resized PNG within a finite request budget. If direct discovery fails, Google supplies the final fallback. Assets stay in Supabase Storage; ICO decoding and Open Graph preview images are excluded. The design and limits are recorded in `docs/DECISIONS.md`.

...
- **API writes and privileged RPCs now enforce their server-side security boundaries.** Authenticated PostgREST and Storage clients can no longer bypass validated mutation routes for profiles, usage, posts, social actions, messages, submissions, or uploads. Privileged RPCs authorize callers explicitly, private feed and usage data use allow-listed fields and ownership joins, and public RPC work is bounded. CLI requests now reject deleted or banned users, image uploads verify file signatures, and profile, search, submission, and pagination inputs have explicit limits.
